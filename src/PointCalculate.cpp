...
int PointCalculate::getAreaPoints(int color){
    ...
    areaData.clear();
    checkedData.clear();
    for(unsigned int y=0;y<height;y++){//配列用意の順番的にxからループを始める
    ...
    for(unsigned int y=1;y<height-1;y++){//フィールドの端のマスは領域ポイントを取らないためループから省く
        for(unsigned int x=1;x<width-1;x++){//横に見ていく、全部見たら次の列、という形
